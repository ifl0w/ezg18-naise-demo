...
//	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	...
	glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, gGlow);
	glTexImage2DMultisample(GL_TEXTURE_2D_MULTISAMPLE, samples, GL_RGBA, width, height, GL_TRUE); //TODO 16F
//	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
//	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D_MULTISAMPLE, gGlow, 0);
...
void DeferredRenderTarget::setTextureUnits(const LightShader& lightShader) {
	...
	glUniform1i(lightShader.diffSpecBufferLocation, 2);
	glActiveTexture(lightShader.diffSpecBufferUnit);
	glBindTexture(GL_TEXTURE_2D_MULTISAMPLE, gAlbedoSpec);

	glUniform1i(lightShader.glowBufferLocation, 4);
