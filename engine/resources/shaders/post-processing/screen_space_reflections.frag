...
/*  Line Rasterization for new position on ray
    ...
    oldCellIndex: pixelcoordinates along the ray between O and D
*/
vec3 intersectCellBoundary(vec3 O, vec3 D, vec2 oldCellIndex, vec2 mipmap_resolution, vec2 crossStep, vec2 crossOffset)
{
	...
	cell += crossOffset;
	vec2 delta = cell - O.xy; // vectore from O to new Cell
	delta /= D.xy; // getting factor between 0 and 1 for interpolating
...
void init(){
    ...
    fragment_depth = texture(gLinearDepth, TexCoords).r; // currently isn't linear because of hi-z and the possibility to interpolate in screenspace!
...
void main(){
    ...
    //reflections = RayMarching(); //currently doesn't work because a linear depth is needed and some variables for hi-z where overwritten
}



void calculateScreenReflection(vec3 view_pos, vec3 view_refl){
    view_endPoint = (view_refl * 1) + view_pos;
...
vec3 hiZTrace(vec3 p, vec3 v){
    ...
    vec3 ray = p; // position on ray, will be updated while stepping along the ray
   ...
   // return vec3(0);
    ...
    while(currentLevel >= HIZ_STOP_LEVEL && currentIterations < HIZ_MAX_ITERATIONS)
    {
        ...
        // MIN of paper: // tmpRay = intersectPlane(o, d, max(ray.z, hiZ_minmax.r));
        ...
        if(crossedCellBoundary(oldCellIdx, newCellIdx))
        {
        	...
        	currentLevel = min(HIZ_MAX_LEVEL, currentLevel + HIZ_ITERATION_STEP); //HIZ_ITERATION_STEP normally should be 2 to compansate the --currentLevel;
        }
