...
void main()
{
    ...
    if(first){
     ...
     /*   minDepth.r = texture( lastImage, TexCoords ).r; // current depth
        ...
        minDepth.a = texelFetch(lastImage, coord + ivec2(offset.x, offset.y), 0).r;

        maxDepth.r = texelFetch(lastImage, coord                            , 0).g;
        maxDepth.g = texelFetch(lastImage, coord + ivec2(offset.x,        0), 0).g;
        maxDepth.b = texelFetch(lastImage, coord + ivec2(  0, offset.y), 0).g;
        maxDepth.a = texelFetch(lastImage, coord + ivec2(offset.x, offset.y), 0).g;
